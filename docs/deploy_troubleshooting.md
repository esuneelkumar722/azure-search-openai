...
1. After running `terraform -chdir=infra/terraform apply -var-file=environments/dev.tfvars` and visiting the website, you see a '404 Not Found' in the browser. Wait 10 minutes and try again, as it might be still starting up. Then try running `az acr build + az containerapp update` and wait again. If you still encounter errors with the deployed app and are deploying to App Service, consult the [guide on debugging App Service deployments](/docs/appservice.md). Please file an issue if the logs don't help you resolve the error.

1. You see a `RoleAssignmentExists` error (HTTP 409) when re-deploying after switching between local development and CI/CD pipelines (or vice versa). This happens because the role assignment GUID changes when the `principalType` changes between `User` and `ServicePrincipal`. Running `terraform -chdir=infra/terraform apply -var-file=environments/dev.tfvars` again should resolve the issue, as the template now generates separate role assignments for each principal type.
